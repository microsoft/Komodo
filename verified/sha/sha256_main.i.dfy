...
import opened Dummy_SHA256Main_types_s = types_s
import opened Dummy_SHA256Main_Math__mod_auto_i = Math__mod_auto_i
import opened Dummy_SHA256Main_Math__div_nonlinear_i = Math__div_nonlinear_i
import opened Dummy_SHA256Main_Math__mul_i = Math__mul_i
import opened Dummy_SHA256Main_Math__power2_i = Math__power2_i
...
lemma lemma_SHA256UpdateWhenNoUnprocessedBytesHelper(
    ...
    requires offset + len <= 0x1_0000_0000;   // TODO: Switch back to 2^64 when not using 32-bit Spartan code
...
method SHA256UpdateWhenNoUnprocessedBytes(
    ...
    requires (offset as int) + (len as int) <= 0x1_0000_0000;   // TODO: Switch back to 2^64 when not using 32-bit Spartan code
...
method SHA256_Init(ctx:SHA256Context)
    requires allocated(ctx);
    requires ctx.Valid();
    ensures  ctx.ExportedInvariant();
...
method {:timeLimitMultiplier 3} SHA256_Update(ctx:SHA256Context, bytes:array<byte>, offset:ulong, len:ulong)
    requires allocated(ctx);
    requires ctx.ExportedInvariant();
...
method SHA256_Complete(bytes:array<byte>, offset:ulong, len:ulong, digest:array<uint>)
    ...
    requires allocated(bytes);
    requires (offset as int) + (len as int) <= bytes.Length;
    requires (offset as int) + (len as int) <= 0x1_0000_0000;   // TODO: Switch back to MaxBytesForSHA() when not using 32-bit Spartan code

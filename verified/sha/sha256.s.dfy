...
function method{:opaque} InitialH_SHA256(j:word) : word
    requires 0 <= j <= 7;
{
...
datatype atoh_Type = atoh_c(a:word, b:word, c:word, d:word, e:word, f:word, g:word, h:word)
datatype SHA256Trace = SHA256Trace_c(M:seq<seq<word>>, H:seq<seq<word>>, W:seq<seq<word>>, atoh:seq<seq<atoh_Type>>)

function ConvertAtoHToSeq(v:atoh_Type) : seq<word>
{
    ...
    requires IsCompleteSHA256Trace(z);
{
    forall blk ::
        0 <= blk < |z.M| ==>
        forall t:word {:trigger TStep(t)} :: TStep(t) ==>
    ...
    requires IsCompleteSHA256Trace(z);
{
    forall blk :: 0 <= blk < |z.M| ==>
        ...
        forall t:word {:trigger TStep(t)} :: TStep(t) && 0 <= t <= 63 ==>
...
predicate DoesTraceDemonstrateSHA256(z:SHA256Trace, message:seq<byte>, hash:seq<word>)
{
...
function {:axiom} SHA256(message:seq<byte>) : seq<word>

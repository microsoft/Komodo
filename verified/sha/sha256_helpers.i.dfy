...
include "../../Math/mod_auto.i.dfy"
include "sha256.i.dfy"
include "sha256_context.i.dfy"
include "../../Util/words_and_bytes.i.dfy"

import opened Dummy_SHA256Helpers_Math__mod_auto_i = Math__mod_auto_i
import opened Dummy_words_and_bytes_i = words_and_bytes_i
...
//    requires IsSHA256ReadyForStep(z, SHA256_state_c(H[..], W[..], atoh), 0);
//    ensures  IsSHA256ReadyForStep(z', SHA256_state_c(H[..], W[..], atoh'), 64);
//    ensures  |z'.H| == |z.H|;
//    ensures  z'.M == z.M;
//    //modifies W;       // BP: I don't think this is true
...
//    ghost z':SHA256Trace
//    )
//    requires current_state.atoh == atoh_c(a, b, c, d, e, f, g, h);
//    requires allocated(W);
//    requires W.Length == 64;
//    requires IsSHA256ReadyForStep(z, current_state, 0);
//    ensures  IsSHA256ReadyForStep(z', current_state.(atoh := atoh'), 64);
//    ensures  |z'.H| == |z.H|;
//    ensures  z'.M == z.M;
...
//method SHA256_ComputeInitialWs(M:array<uint>, W:array<uint>)
//    requires allocated(M);
//    requires M.Length == 16;
//    requires allocated(W);
//    requires W.Length == 64;
//    ensures forall t:uint32 {:trigger TStep(t)} :: TStep(t) && 0 <= t < 16 ==> W[t] == M[t];
//    ensures forall t:uint32 {:trigger TStep(t)} :: TStep(t) && 16 <= t <= 63 ==> W[t] == BitwiseAdd32(BitwiseAdd32(BitwiseAdd32(SSIG1(W[t-2]), W[t-7]), SSIG0(W[t-15])), W[t-16]);

method SHA256_ComputeInitialWs(ptr:array<byte>, ptr_offset:uint, W:array<uint>)
    requires allocated(ptr);
...
method {:timeLimitMultiplier 8} SHA256_DigestOneBlock(ctx:SHA256Context, W:array<uint>, ptr:array<byte>, offset:ulong)
    ...
    requires (offset as int) + 64 <= 0x1_0000_0000;         // TODO: We need this restriction when using 32-bit Spartan code
    ensures  ctx.Valid();
    ...
    modifies ctx, ctx.H, W;
{
    ...
    SHA256_ComputeInitialWs(ptr, offset as uint, W);
...
method SHA256_BlockDataOrder(ctx:SHA256Context, ptr:array<byte>, offset:ulong, len:ulong)
    requires allocated(ctx);
    requires ctx.Valid();
    requires |ctx.processed_bytes| % 64 == 0;
    requires IsCompleteSHA256Trace(ctx.z);
    ...
    requires (offset as int) + (len as int) <= 0x1_0000_0000;  // TODO: We need this restriction when using 32-bit Spartan code
    ensures  ctx.Valid();
    ensures  IsCompleteSHA256Trace(ctx.z);
    ensures  SHA256TraceIsCorrect(ctx.z);
    ensures  ctx.H == old(ctx.H);

...
lemma {:axiom} lemma_decl_svc_r0(s13:state, s14:state, s23:state, s24:state)
    ...
    requires ValidState(s23) && ValidState(s24)
    requires mode_of_state(s13) == mode_of_state(s23) == User
    requires s13.conf.nondet == s23.conf.nondet
    requires exists pc1:word :: evalExceptionTaken(s13, ExSVC, pc1, s14)
    requires exists pc2:word :: evalExceptionTaken(s23, ExSVC, pc2, s24)
    ensures (reveal_ValidRegState(); s14.regs[R0] == s24.regs[R0])
...
lemma {:axiom} lemma_decl_svc_r1(s13:state, s14:state, s23:state, s24:state)
    ...
    requires exists pc1:word :: evalExceptionTaken(s13, ExSVC, pc1, s14)
    requires exists pc2:word :: evalExceptionTaken(s23, ExSVC, pc2, s24)
    requires (reveal_ValidRegState(); s14.regs[R0] == s24.regs[R0])
    ...
    // spare page.
    ensures (reveal_ValidRegState(); s14.regs[R1] == s24.regs[R1])
...
lemma {:axiom} lemma_decl_svc_r2(s13:state, s14:state, s23:state, s24:state)
    ...
    ensures R2 in s14.regs && R0 in s24.regs
    ensures (reveal_ValidRegState(); s14.regs[R2] == s24.regs[R2])

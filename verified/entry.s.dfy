...
predicate svc(s:SysState,s':SysState) 
{
    ...
    s'.hw.conf.cpsr.m == Monitor && s'.hw.regs[R0] == KEV_ERR_SUCCESS() &&
...
{
    userEnteredState(s) && userEnteredState(s') && s'.hw.conf.cpsr.m == Monitor &&
    ...
    s.hw.conf.spsr[mode_of_state(s.hw)].m == User
}

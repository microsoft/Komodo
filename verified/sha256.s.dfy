    ...
    requires IsCompleteSHA256Trace(z);
{
    ...
    (forall blk:uint32 {:trigger TBlk(blk)} :: TBlk(blk) ==> forall j ::
        0 <= int(blk) < |z.M| && 0 <= j < 8 ==> z.H[int(blk)+1][j] == BitwiseAdd32(ConvertAtoHToSeq(z.atoh[blk][64])[j], z.H[blk][j]))
}
...
//
//predicate {:autoReq} IsSHA256(messageBits:seq<int>, hash:seq<int>)
//{
...
//}
//
//function {:axiom} SHA256(messageBits:seq<int>) : seq<int>

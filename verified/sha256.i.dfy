...
datatype SHA256_state = SHA256_state_c(M:seq<uint32>, H:seq<uint32>, W:seq<uint32>, atoh:atoh_Type, num_blocks:uint32)
function SHA256_vars_to_state(M:array<uint32>, words:uint32, H:array<uint32>, W:array<uint32>, atoh:atoh_Type, num_blocks:uint32)
                                    : SHA256_state
...
predicate CorrectlyAccumulatedHsForBlock(z:SHA256Trace, blk:uint32)
    requires int(blk) < |z.atoh| && 64 < |z.atoh[blk]| && int(blk) + 1 < |z.H|;
{
    forall j {:trigger TStep(j)}:: 0 <= j < 8 && |z.H[blk]| == |z.H[int(blk)+1]| == 8 ==> 
        z.H[int(blk)+1][j] == BitwiseAdd32(ConvertAtoHToSeq(z.atoh[blk][64])[j], z.H[blk][j])
}

predicate CorrectlyAccumulatedHsForAllBlocks(z:SHA256Trace)
    requires |z.H|-1 <= |z.atoh|; 
{
    forall blk {:trigger CorrectlyAccumulatedHsForBlock(z, blk)} :: 0 <= int(blk) < |z.H|-1 ==>
...
//    (forall blk {:trigger TBlk(blk)} :: TBlk(blk) && 0 <= int(blk) < |z.H|-1 ==>
//        |z.atoh[blk]| == 65 && CorrectlyAccumulatedHs(z, blk)
        //forall j {:trigger TStep(j)}:: 0 <= j < 8 ==> z.H[int(blk)+1][j] == BitwiseAdd32(ConvertAtoHToSeq(z.atoh[blk][64])[j], z.H[blk][j]))
}
...
predicate PartialSHA256TraceHasCorrectatohsWf(z:SHA256Trace)
{
    ...
    forall blk:uint32 {:trigger TBlk(blk)}:: 0 <= int(blk) < |z.atoh| ==>
        |z.atoh[blk]| <= 65 &&
        |z.W[blk]| == 64 &&
...
predicate{:opaque} PartialSHA256TraceHasCorrectatohsOpaque(z:SHA256Trace)
{
    ...
    forall blk :: 0 <= blk < |z.atoh| ==>
        ...
        |z.W[blk]| == 64 &&
        (|z.atoh[blk]| > 0 ==> |z.H[blk]| == 8 && ConvertAtoHToSeq(z.atoh[blk][0]) == z.H[blk]) &&
        forall t {:trigger TStep(t)} :: TStep(t) && 0 <= int(t) < |z.atoh[blk]|-1 ==>
...
predicate AreSHA256TraceAndStateOK(z:SHA256Trace, s:SHA256_state)
{
    PartialSHA256TraceIsCorrect(z) &&   
    z.M == BreakIntoBlocks(s.M, 16) &&
    (forall i {:trigger |z.M[i]|}:: 0 <= i < |z.M| ==> |z.M[i]| == 16) &&
    16*|z.M| == |s.M| &&
    |z.M| == int(s.num_blocks)
}

predicate IsSHA256ReadyForStep(z:SHA256Trace, s:SHA256_state, currentBlock:int, nextStep:int)
    ...
    requires 0 <= nextStep <= 64;
{
    ...
    (forall blk {:trigger |z.atoh[blk]|}:: 0 <= blk < currentBlock ==> |z.atoh[blk]| == 65) &&
    |z.atoh[currentBlock]| == nextStep+1 &&
    s.H == z.H[currentBlock] &&
    s.W == z.W[currentBlock] &&
    s.atoh == z.atoh[currentBlock][nextStep]
}

predicate{:opaque} TheAToHsAreOK(z:SHA256Trace, blk:uint32, t:uint32)
    requires 0 <= t <= 63;
    requires 0 <= blk;
    requires |z.atoh| > int(blk);
    requires |z.atoh[blk]| > int(t+1);
    requires |z.W| > int(blk);
    requires |z.W[blk]| == 64;
{
    var T1 := BitwiseAdd32(BitwiseAdd32(BitwiseAdd32(BitwiseAdd32(z.atoh[blk][t].h, BSIG1(z.atoh[blk][t].e)),
                                      Ch(z.atoh[blk][t].e, z.atoh[blk][t].f, z.atoh[blk][t].g)),
...
lemma lemma_SHA256TransitionOKAfterSettingAtoHStep1Helper1(z:SHA256Trace, blk:uint32, t:uint32)
    requires 0 <= t <= 63;
    requires 0 <= blk;
    requires |z.atoh| > int(blk);
    requires |z.atoh[blk]| > int(t+1);
...
lemma Lemma_TheAToHsAreOKIsStable(z1:SHA256Trace, z2:SHA256Trace, blk:uint32, t:uint32)
    requires 0 <= t <= 63;
    requires 0 <= blk;
    requires |z1.atoh| == |z2.atoh| > int(blk);
    requires |z1.atoh[blk]| > int(t+1);
    requires |z2.atoh[blk]| > int(t+1);
    requires z1.atoh[blk][t+1] == z2.atoh[blk][t+1];
    requires z1.atoh[blk][t] == z2.atoh[blk][t];
    requires |z1.W| > int(blk);
    requires z1.W == z2.W;
...
lemma {:timeLimitMultiplier 2} lemma_SHA256TransitionOKAfterSettingAtoHStep1(z1:SHA256Trace, s1:SHA256_state, z2:SHA256Trace, s2:SHA256_state,
                                                                                    currentBlock:uint32, currentStep:uint32)
    requires 0 <= int(currentBlock) < |z1.M|;
    requires 0 <= currentStep <= 63;
    requires IsSHA256ReadyForStep(z1, s1, int(currentBlock), int(currentStep));
    ...
    ensures forall blk:uint32 :: 0 <= int(blk) < |z2.atoh| ==>
        |z2.atoh[blk]| <= |z2.W[blk]| + 1 &&
        |z2.atoh[blk]| <= 65 &&
        (|z2.atoh[blk]| > 0 ==> |z2.H[blk]| == 8 && ConvertAtoHToSeq(z2.atoh[blk][0]) == z2.H[blk]) &&
        (forall t:uint32 :: 0 <= int(t) < |z2.atoh[blk]|-1 ==> TheAToHsAreOK(z2, blk, t));
{
    forall blk | TBlk(blk) && 0 <= int(blk) < |z2.atoh|
        ...
        ensures forall t:uint32 :: 0 <= int(t) < |z2.atoh[blk]|-1 ==> TheAToHsAreOK(z2, blk, t);
    {
        assert |z2.atoh[blk]| <= |z2.W[blk]| + 1;
        assert |z2.atoh[blk]| <= 65;

        if int(blk) < |z2.atoh|-1 {
            ...
            forall t:uint32 | 0 <= int(t) < |z1.atoh[blk]|-1
                ensures TheAToHsAreOK(z2, blk, t);
            {
                lemma_SHA256TransitionOKAfterSettingAtoHStep1Helper1(z1, blk, t);
                Lemma_TheAToHsAreOKIsStable(z1, z2, blk, t);
            }
            assert forall t:uint32 :: 0 <= int(t) < |z2.atoh[blk]|-1 ==> TheAToHsAreOK(z2, blk, t);
        }
        else {
            assert blk == currentBlock;
            assert (|z2.atoh[blk]| > 0 ==> |z2.H[blk]| == 8 && ConvertAtoHToSeq(z2.atoh[blk][0]) == z2.H[blk]);
            forall t:uint32 | 0 <= int(t) < |z2.atoh[blk]|-1
                ensures TheAToHsAreOK(z2, blk, t);
            {
                if int(t) < |z2.atoh[blk]|-2 {
...
lemma lemma_SHA256TransitionOKAfterSettingAtoH(z1:SHA256Trace, s1:SHA256_state, z2:SHA256Trace, s2:SHA256_state,
                                                      currentBlock:uint32, currentStep:uint32)
    requires 0 <= int(currentBlock) < |z1.M|;
    requires 0 <= currentStep <= 63;
    requires IsSHA256ReadyForStep(z1, s1, int(currentBlock), int(currentStep));
    ...
    ensures IsSHA256ReadyForStep(z2, s2, int(currentBlock), int(currentStep+1));
{
    lemma_SHA256TransitionOKAfterSettingAtoHStep1(z1, s1, z2, s2, currentBlock, currentStep);

    assert forall blk:uint32 :: 0 <= int(blk) < |z2.atoh| ==>
        |z2.atoh[blk]| <= |z2.W[blk]| + 1 &&
        |z2.atoh[blk]| <= 65 &&
        (|z2.atoh[blk]| > 0 ==> |z2.H[blk]| == 8 && ConvertAtoHToSeq(z2.atoh[blk][0]) == z2.H[blk]) &&
        (forall t:uint32 :: 0 <= int(t) < |z2.atoh[blk]|-1 ==> TheAToHsAreOK(z2, blk, t));

    forall blk:uint32 | 0 <= int(blk) < |z2.atoh|
        ensures forall t:uint32 {:trigger TStep(t)} :: TStep(t) && 0 <= int(t) < |z2.atoh[blk]|-1 ==>
    ...
    {
        forall t:uint32 {:trigger TStep(t)} | TStep(t) && 0 <= int(t) < |z2.atoh[blk]|-1
    ...
    reveal_PartialSHA256TraceHasCorrectatohsOpaque();
    assert TBlk(currentBlock);
    assert TStep(currentStep);


    forall blk {:trigger CorrectlyAccumulatedHsForBlock(z2, blk)} | 0 <= int(blk) < |z2.H|-1 
        ensures |z2.atoh[blk]| == 65 && CorrectlyAccumulatedHsForBlock(z2, blk);
    {
        assert CorrectlyAccumulatedHsForBlock(z1, blk);
        forall j | 0 <= j < 8 && |z1.H[blk]| == |z1.H[int(blk)+1]| == 8
            ensures z1.H[int(blk)+1][j] == BitwiseAdd32(ConvertAtoHToSeq(z1.atoh[blk][64])[j], z1.H[blk][j])
        {
            assert TStep(j);
        }
        forall j | 0 <= j < 8 && |z2.H[blk]| == |z2.H[int(blk)+1]| == 8 
            ensures z2.H[int(blk)+1][j] == BitwiseAdd32(ConvertAtoHToSeq(z2.atoh[blk][64])[j], z2.H[blk][j])
        {
            assert TStep(j);
        }
...
method ComputeOneStep_SHA256(M:array<uint32>, words:uint32, H:array<uint32>, W:array<uint32>, ghost atoh:atoh_Type, num_blocks:uint32, 
    ...
    requires IsSHA256ReadyForStep(z, SHA256_vars_to_state(M, words, H, W, atoh, num_blocks), int(currentBlock), int(currentStep));
    ensures IsSHA256ReadyForStep(next_z, SHA256_vars_to_state(M, words, H, W, next_atoh, num_blocks), int(currentBlock), int(currentStep)+1);
...
method ComputeOneStep_SHA256_optimized_clean(ghost currentState:SHA256_state, ghost atoh:atoh_Type, W:uint32, ghost num_blocks:uint32,
    ...
    requires 0 <= int(currentStep) < |currentState.W| && currentState.W[currentStep] == W;
